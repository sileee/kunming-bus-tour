...
| 数据模式 | File 演示模式 (默认) / MySQL 持久化模式 (生产) |
| 交付物 | 前端 SPA、REST API 服务、数据库 DDL/DML、接口文档、测试报告、成果 PPT |
...
系统采用前后端分离架构。前端分为游客端（`/tourist.html`，手绘美学，无需登录）和管理端（`/`，科技风大屏，需 JWT 登录）；后端负责接口封装、JWT 认证、数据查询与采集样本写入；数据访问层支持 File/Mysql 双模式。

```mermaid
flowchart TB
  subgraph Client["客户端 (Browser)"]
    Tourist["游客导览页 (tourist.html)<br/>手绘 scrapbook 美学<br/>地图/景点/线路 · 无需登录"]
    Admin["管理平台 (index.html)<br/>科技风大屏<br/>需 JWT 登录 · 7 个 Tab"]
  end
...
├── frontend/                             # Vue 3 前端
│   ├── package.json
│   ├── vite.config.ts                    # Vite 多页面构建 (main + tourist)
│   ├── tsconfig.json
│   ├── index.html                        # 管理平台入口
...
│       ├── api.ts                        # REST API 封装 (含 JWT Token 注入)
...
| 类型 | 路径 | 说明 |
| --- | --- | --- |
| README | `README.md` | 项目介绍与快速启动指南（V5.0） |
| 需求分析 | `docs/需求分析.md` | 业务需求与功能分析 |
| 概要设计 | `docs/概要设计.md` | 系统架构与模块设计 |
| 开发文档 | `docs/开发文档.md` | 本文档（V5.0，含完整迭代记录） |
| 接口文档 | `docs/接口文档.md` | 全部 API 端点详细说明（V5.0，含认证章节） |
...
| 前期准备 | `docs/前期准备.md` | 愿景/技术栈/参考项目/小组规划（V5.0） |
